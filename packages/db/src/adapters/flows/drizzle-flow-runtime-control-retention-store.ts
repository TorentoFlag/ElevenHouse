...
export type FlowRuntimeControlOutcomeRetentionResult = {
  readonly purged: number;
};

export async function runFlowRuntimeControlOutcomeRetention(
  database: ElevenHouseDatabase,
  input: { readonly batchSize: number }
): Promise<FlowRuntimeControlOutcomeRetentionResult> {
    ...
    const result = await transaction.execute(sql<PurgedOutcomeRow>`
      with candidates as (
        select outcome.command_id
          from ${flowRuntimeControlCommandOutcomes} outcome
          join ${flowRuntimeControlCommands} command
            on command.id = outcome.command_id
      ...
      delete from ${flowRuntimeControlCommandOutcomes} outcome
       using candidates
