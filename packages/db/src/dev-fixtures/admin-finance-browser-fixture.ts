...
export async function seedAdminFinanceBrowserFixture(
  ...
  try {
    ...
    await seedProductAndChargebackPayment(client);
...
async function seedUsersAndAdminSession(client: Queryable): Promise<void> {
  ...
  const sessionTokenHash = hashSessionToken(fixture.sessionToken);
  await query(client, "delete from user_sessions where token_hash = $1 and id <> $2", [
    sessionTokenHash,
    fixture.sessionId
  ]);
  await query(
    ...
    `insert into user_sessions
     ...
     values ($1, $2, $3, 'active', 'ElevenHouse dev finance fixture', '127.0.0.1',
       $4, $4, $5, null)
     on conflict (id) do update
     set user_id = excluded.user_id,
         ...
         revoked_at = null`,
    [fixture.sessionId, fixture.adminUserId, sessionTokenHash, now, sessionExpiresAt]
  );
  await query(
    client,
    `insert into auth_security_events
       (id, user_id, session_id, event_type, occurred_at, ip_address, user_agent, metadata)
     values ($1, $2, $3, 'login_succeeded', $4, '127.0.0.1',
    ...
    [fixture.authSecurityEventId, fixture.adminUserId, fixture.sessionId, now]
  );
}
...
async function seedFinancePolicy(client: Queryable): Promise<void> {
  ...
  await query(
    ...
    `insert into finance_policies
     ...
     values ($1, 970001, 'manual_review', 48, 0, 0, 1000, true, true, $2, $3, $3)
     on conflict (id) do update
...
async function seedProductAndChargebackPayment(client: Queryable): Promise<void> {
  await query(
    client,
    `insert into products
     ...
     values ($1, $2, 'single', 'active', 'Dev chargeback consultation', 50000, 'RUB',
  ...
  await query(
    ...
    `insert into orders
     ...
     values ($1, $2, $3, $4, 'chargeback', 50000, 'RUB', 5000, 'RUB', 45000, 'RUB',
       $5, 'manual_review', 48, 0, 0, 1000, true, $6, $6)
     on conflict (id) do update
  ...
  await query(
    ...
    `insert into payment_attempts
     ...
     values ($1, $2, 'arc_pay', 'sandbox', 'chargeback', 50000, 'RUB',
     ...
     set status = excluded.status,
         metadata = excluded.metadata,
         updated_at = excluded.updated_at`,
    [fixture.chargebackPaymentAttemptId, fixture.chargebackOrderId, now]
  );
  await query(
    client,
...
async function seedLedger(client: Queryable): Promise<void> {
  await createLedgerTransaction(client, {
    id: fixture.fundingLedgerTransactionId,
    operationType: "manual_adjustment",
    orderId: null,
    payoutRequestId: null,
    occurredAt: "2026-07-28T08:50:00.000Z",
  ...
  await createLedgerTransaction(client, {
    ...
    metadata: { source: "seed-dev-admin-finance", payoutMethodId: fixture.payoutMethodId },
    entries: [
      entry("astrologer_available", fixture.astrologerUserId, "debit", 45_000),
      entry("astrologer_payout_pending", fixture.astrologerUserId, "credit", 45_000)
    ]
  });
  await createLedgerTransaction(client, {
    ...
    entries: [
      entry("astrologer_payout_pending", fixture.astrologerUserId, "debit", 45_000),
      entry("astrologer_available", fixture.astrologerUserId, "credit", 45_000)
    ]
  });
  await createLedgerTransaction(client, {
    ...
    entries: [
      entry("platform_revenue", null, "debit", 5_000),
      entry("astrologer_available", fixture.astrologerUserId, "debit", 45_000),
      entry("platform_clearing", null, "credit", 50_000)
    ]
  });
}
...
function entry(
  ...
  return {
    ...
    side,
    amountMinor,
    metadata: { source: "seed-dev-admin-finance" }
  };
}
