...
BEGIN
  ...
  IF NEW.event_type = 'token_advanced' AND NEW.summary->>'reasonCode' = 'FLOW_APPROVAL_DECIDED' AND (
    ...
    OR NEW.summary->>'nodeKind' NOT IN ('astrologer_approval', 'natal_chart_ai_draft')
...
DO $flow_terminal_node_constraints$
DECLARE
...
BEGIN
  FOR constraint_name, table_name IN
    SELECT * FROM (VALUES
      ('flow_execution_attempts_trace_summary_schema_check', 'flow_execution_attempts'),
      ('flow_run_events_summary_schema_check', 'flow_run_events')
    ) AS constraints(constraint_name, table_name)
  LOOP
    IF table_name = 'flow_execution_attempts' THEN
      target := $node_kind$((trace_summary ->> 'nodeKind'::text) = 'completed'::text)$node_kind$;
      replacement := $node_kind$((trace_summary ->> 'nodeKind'::text) = ANY (ARRAY['completed'::text, 'suppressed'::text, 'failed'::text]))$node_kind$;
    ELSE
      target := $node_kind$((summary ->> 'nodeKind'::text) = 'completed'::text)$node_kind$;
      replacement := $node_kind$((summary ->> 'nodeKind'::text) = ANY (ARRAY['completed'::text, 'suppressed'::text, 'failed'::text]))$node_kind$;
    END IF;
    ...
    IF definition IS NULL THEN
      RAISE EXCEPTION 'expected flow terminal constraint % on %', constraint_name, table_name;
    END IF;
    occurrences := (length(definition) - length(replace(definition, target, ''))) / length(target);
    IF occurrences <> 1 THEN
      RAISE EXCEPTION 'expected exactly one terminal node predicate in % on %, got %', constraint_name, table_name, occurrences;
    END IF;
    ...
    EXECUTE format(
      ...
      table_name,
      constraint_name,
      replace(definition, target, replacement)
    );
  END LOOP;
END;
$flow_terminal_node_constraints$;
