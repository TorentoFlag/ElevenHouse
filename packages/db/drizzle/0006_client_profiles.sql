...
CREATE TABLE "client_birth_data" (
	...
	CONSTRAINT "client_birth_data_source_check" CHECK ("client_birth_data"."source" in ('client_profile', 'import', 'manual')),
	CONSTRAINT "client_birth_data_last_edited_by_role_check" CHECK ("client_birth_data"."last_edited_by_role" in ('client', 'astrologer')),
	...
	CONSTRAINT "client_birth_data_revision_check" CHECK ("client_birth_data"."revision" >= 1)
);
--> statement-breakpoint
CREATE TABLE "client_birth_data_history" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
	...
	CONSTRAINT "client_birth_data_history_birth_data_revision_unique" UNIQUE("birth_data_id","revision"),
	CONSTRAINT "client_birth_data_history_revision_check" CHECK ("client_birth_data_history"."revision" >= 1),
	CONSTRAINT "client_birth_data_history_actor_role_check" CHECK ("client_birth_data_history"."actor_role" in ('client', 'astrologer')),
	CONSTRAINT "client_birth_data_history_source_check" CHECK ("client_birth_data_history"."source" in ('client_profile', 'import', 'manual'))
);
--> statement-breakpoint
...
CREATE INDEX "client_birth_data_history_client_recorded_idx" ON "client_birth_data_history" USING btree ("client_user_id","recorded_at");--> statement-breakpoint
...
--> statement-breakpoint
-- ElevenHouse client birth-profile integrity objects: begin
CREATE OR REPLACE FUNCTION elevenhouse_reject_client_birth_data_history_mutation()
RETURNS trigger
LANGUAGE plpgsql
...
FOR EACH STATEMENT
EXECUTE FUNCTION elevenhouse_reject_client_birth_data_history_mutation();
-- ElevenHouse client birth-profile integrity objects: end
