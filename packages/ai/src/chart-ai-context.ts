...
export function getNatalChartAiDictionaryCodes(result: ReproducibleNatalChartResult): readonly string[] {
  const codes: string[] = [];
  ...
  for (const aspect of selectMajorAspects(result.result.aspects)) {
    ...
    codes.push(`aspect_${normalizeCodePart(pointA)}_${normalizeAspectTypeCode(aspect.type)}_${normalizeCodePart(pointB)}`);
  }
  return Array.from(new Set(codes));
}

export function buildNatalChartAiContext(input: ChartAiContextInput): ChartInterpretationDraftPromptInput {
  const zodiac = input.result.settings.zodiac;
  if (zodiac !== "tropical") throw new Error("Stored natal result must declare the tropical zodiac setting");
  const groundingByCode = new Map(
    input.dictionaryEntries.filter((entry) => entry.source === "platform").map((entry) => [entry.code, entry])
  );
  ...
  return {
    ...
    points: sortPoints(input.result.result.points).map((point) => ({
      id: point.id, label: point.label, sign: point.sign, degree: round(point.signDegree),
      house: point.house ?? null, retrograde: point.retrograde ?? false
    })),
    houses: [...input.result.result.houses].sort((left, right) => left.number - right.number).map((house) => ({
      number: house.number, sign: house.sign, degree: round(house.signDegree)
    })),
    majorAspects: selectMajorAspects(input.result.result.aspects).map((aspect) => ({
      pointA: aspect.pointA, pointB: aspect.pointB, type: aspect.type, orb: round(aspect.orb),
      applying: aspect.applying ?? null, strength: aspect.strength ?? null
    })),
...
function sortPoints(points: readonly ChartPoint[]): readonly ChartPoint[] {
  ...
  return [...points].sort((left, right) => (order.get(left.id) ?? 100) - (order.get(right.id) ?? 100));
}
function selectMajorAspects(aspects: readonly ChartAspect[]): readonly ChartAspect[] { return [...aspects].sort(compareAspects).slice(0, maxAspects); }
function compareAspects(left: ChartAspect, right: ChartAspect): number { const strength = (right.strength ?? 0) - (left.strength ?? 0); return strength !== 0 ? strength : left.orb - right.orb; }
function isDictionaryAspect(aspect: ChartAspect): boolean { return aspectPointIds.has(normalizeCodePart(aspect.pointA) as (typeof pointOrder)[number]) && aspectPointIds.has(normalizeCodePart(aspect.pointB) as (typeof pointOrder)[number]); }
function orderAspectPair(left: string, right: string): readonly [string, string] { const leftIndex = pointOrder.indexOf(normalizeCodePart(left) as (typeof pointOrder)[number]); const rightIndex = pointOrder.indexOf(normalizeCodePart(right) as (typeof pointOrder)[number]); return leftIndex === -1 || rightIndex === -1 || leftIndex <= rightIndex ? [left, right] : [right, left]; }
function normalizeAspectTypeCode(type: string): string { return normalizeCodePart(type).replace(/^opposition$/, "opposition"); }
function normalizeCodePart(value: string): string { return value.trim().toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, ""); }
function trimText(value: string, maxLength: number): string { const normalized = value.trim().replace(/\s+/g, " "); return normalized.length > maxLength ? `${normalized.slice(0, maxLength - 1).trim()}…` : normalized; }
