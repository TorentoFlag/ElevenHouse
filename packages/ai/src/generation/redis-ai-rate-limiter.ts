...
export type AiRateLimitBucketOptions = { readonly limit: number; readonly windowSeconds: number };
...
export type RedisAiRateLimitClient = {
  readonly eval: (script: string, options: { readonly keys: string[]; readonly arguments: string[] }) => Promise<unknown>;
};
...
export class RedisAiRateLimiter implements AiRateLimiterPort {
  ...
  async consume(input: { readonly ownerUserId: string }): Promise<AiRateLimitDecision> {
    const ownerHash = createHash("sha256").update(input.ownerUserId).digest("hex");
    const buckets = [
    ...
    const result = await this.client.eval(redisAiRateLimitScript, {
      keys: buckets.map((bucket) => bucket.key),
      arguments: [
        this.now().getTime().toString(),
        this.nonce(),
        ...buckets.flatMap((bucket) => [bucket.limit.toString(), (bucket.windowSeconds * 1000).toString()])
      ]
    });
    const retryAfterSeconds = parseResult(result);
    return retryAfterSeconds > 0 ? { allowed: false, retryAfterSeconds } : { allowed: true };
  }

  private key(...parts: readonly string[]): string { return [`{${this.keyPrefix}}`, ...parts].join(":"); }
}

function parseResult(result: unknown): number {
  const parsed = typeof result === "bigint" ? Number(result) : typeof result === "string" ? Number(result) : result;
...
for i = 1, #KEYS do
  ...
  if count >= limit then
    ...
    if oldest[2] ~= nil then retry_after_ms = tonumber(oldest[2]) + window_ms - now end
    if retry_after_ms > blocked_retry_after_ms then blocked_retry_after_ms = retry_after_ms end
  end
end

if blocked_retry_after_ms > 0 then return math.max(1, math.ceil(blocked_retry_after_ms / 1000)) end
