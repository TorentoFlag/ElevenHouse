...
| --- | --- | --- |
| Public astrologer page (`page.jsx`, `page-data.jsx`) | `client-web` direct-link routes backed by `public-api`; `AstrologerProfile`, `PublicPage`, `Products`, `Availability`, `Reviews`, `LeadMagnets`, `Content`, `Promotions` | `partial`: `/a/:handle` creates/resolves a join intent, exposes safe public identity, carries pending context through client auth and renders approved visible reviews with rating summary, star filters, all-reviews dialog and approved astrologer replies. Full product/availability/lead-magnet reads are still separate missing contours; never expose discovery or a public catalogue. |
...
| Sessions/materials (`client.jsx`, `session-call.jsx`) | Authenticated `/me` and `/sessions/:sessionId`; `Sessions`, `Bookings`, `Orders`, future `Recordings`, `Materials`, `Media`, `Consent` | `partial`: relationship-scoped Session list, 1:1 LiveKit audio/video, screen share, durable chat and reconnect exist. Slice A is explicitly not recorded; recording playback and materials remain missing and require consent, retention and access control. |
