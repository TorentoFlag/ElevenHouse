...
| Numerology                          | `numerology.jsx`, `numerology-data.jsx`, `numerology-extra.jsx`                                                                                                                                                                   | `/numerology` in `astrologer-web`                                                      | `Calculations`, `Numerology`, `Clients`, `AstrologerProfile`, future `Ai`, `Media`                                                                                                             | canonical Pythagorean RU preview/persist/recalculate contracts and owner-scoped storage ready; complete typed compatibility result includes 5 key, 9 matrix, and 8 line comparisons, 4 zones, relation counts, and conclusion; see `docs/superpowers/specs/2026-07-14-pythagorean-ru-correction-design.md` | Phase 1 complete in code, automated verification, and the authorized Chrome flow through Computer Use: production-backed individual and complete compatibility workspace; period selection, full lifecycle UI, presentation completion, AI, and PDF remain unavailable | Canonical toolbar, three-column result, compatibility matrices, server-provided year/month values                                                           | Visible design is preserved. Client selection uses read-only preview; explicit link persists the current result. One canonical Pythagorean engine is active, without method/result version history or frontend arithmetic. Line values remain raw Pythagorean counts, not 1-10 scores. Future methods must be added as separate engines behind the registry and typed contracts.                              |
| Destiny matrix                      | `matrix.jsx`, `matrix-data.jsx`, `matrix-graph.jsx`, `matrix-modes.jsx`                                                                                                                                                           | future `/destiny-matrix` in `astrologer-web`                                           | `Calculations`, `Matrix`, `Clients`, `AstrologerProfile`, future `Ai`, `Media`                                                                                                                  | backend calculation core, private notes and revisioned RU/EN interpretation catalog ready; report/AI/PDF missing                      | missing                                                                                                  | Matrix graph, section navigation, energy table, year selector, notes/report panels                                                                         | CRM-only `ladini_22`; note staleness is checksum-derived and all notes remain private to the astrologer; see the production design spec.                                                                                                                                          |
...
reads have no persistence or AI side effect. Report editing, AI, PDF and the
frontend remain missing; the full decisions live in
