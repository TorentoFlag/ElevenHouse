...
export function createPasswordlessCodeRequest(
  ...
  return requestAstrologerPasswordlessCodeRequestSchema.parse(identifier);
}
...
export function createPasswordlessVerificationRequest(input: {
...
}): VerifyAstrologerPasswordlessCodeRequest | VerifyAstrologerRegistrationPasswordlessCodeRequest {
  if (input.mode === "register") {
    return verifyAstrologerRegistrationPasswordlessCodeRequestSchema.parse({
  ...
  return verifyAstrologerPasswordlessCodeRequestSchema.parse({
    challengeId: input.challengeId,
    code: input.code
  });
}
