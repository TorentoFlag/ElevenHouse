...
const testState = vi.hoisted(() => ({
  draft: undefined as ReferenceEntryModalDraft | undefined,
  setDraft: vi.fn(
    (
      nextDraft:
        | ReferenceEntryModalDraft
      ...
      testState.draft =
        typeof nextDraft === "function" ? nextDraft(testState.draft) : nextDraft;
    }
  ),
...
vi.mock("react", async (importOriginal) => {
  ...
  return {
    ...actual,
    useState: vi.fn(
      (initializer: ReferenceEntryModalDraft | (() => ReferenceEntryModalDraft)) => {
        if (!testState.draft) {
          testState.draft = typeof initializer === "function" ? initializer() : initializer;
        }

        return [testState.draft, testState.setDraft] as const;
      }
    )
  };
});
