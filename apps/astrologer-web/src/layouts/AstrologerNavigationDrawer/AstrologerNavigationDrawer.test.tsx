...
describe("AstrologerNavigationDrawerView", () => {
  it("maps astrologer navigation copy to the shared navigation drawer", () => {
    ...
    expect(element.props.items).toHaveLength(11);
    ...
    expect(element.props.items[2].icon.props.iconName).toBe("wallet");
    ...
    expect(element.props.items[3].icon.props.iconName).toBe("box");
    ...
    expect(element.props.items[4].icon.props.iconName).toBe("orbit");
    ...
    expect(element.props.items[5].icon.props.iconName).toBe("numerology");
    ...
    expect(element.props.items[6].icon.props.iconName).toBe("orbit");
    expect(element.props.items[7]).toMatchObject({
      id: "humanDesign",
      title: "Дизайн человека",
      href: "/human-design"
    });
    expect(element.props.items[7].icon.type).toBe(Icon);
    expect(element.props.items[7].icon.props.iconName).toBe("flow");
    expect(element.props.items[8]).toMatchObject({
      id: "astroCalendar",
      title: "Астрокалендарь",
      href: "/astro-calendar"
    });
    expect(element.props.items[8].icon.type).toBe(Icon);
    expect(element.props.items[8].icon.props.iconName).toBe("orbit");
    expect(element.props.items[9]).toMatchObject({
      id: "reference",
      title: "Справочники",
      href: "/reference"
    });
    expect(element.props.items[9].icon.type).toBe(Icon);
    expect(element.props.items[9].icon.props.iconName).toBe("reference");
    ...
    expect(element.props.items[10].icon.props.iconName).toBe("chat");
