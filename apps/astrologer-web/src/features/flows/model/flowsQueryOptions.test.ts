...
describe("flows query options", () => {
  it("creates stable query keys for flow lists and templates", () => {
    const query = { status: "draft", limit: 20, offset: 0 } as const;
    ...
    expect(flowsQueryKeys.templates()).toEqual(["flows", "templates"]);
    ...
    expect(flowTemplatesQueryOptions().queryKey).toEqual(["flows", "templates"]);
    expect(
      flowRunsQueryOptions("11111111-1111-4111-8111-111111111111", runQuery).queryKey
    ).toEqual(["flows", "runs", "11111111-1111-4111-8111-111111111111", runQuery]);
    expect(flowApprovalsQueryOptions({ status: "pending", limit: 50, offset: 0 }).queryKey).toEqual([
      "flows",
      "approvals",
      { status: "pending", limit: 50, offset: 0 }
    ]);
    expect(flowRunsQueryOptions("11111111-1111-4111-8111-111111111111", runQuery)).not.toHaveProperty(
      "placeholderData"
    );
  });
  ...
  it("creates React Query hooks for flow reads and mutations", () => {
    expect(useFlowListQuery({ status: "all", limit: 50, offset: 0 })).toHaveProperty("queryFn");
    expect(useFlowTemplatesQuery()).toHaveProperty("queryFn");
