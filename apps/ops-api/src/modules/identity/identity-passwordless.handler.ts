...
export const OPS_AUTH_CODE_GENERATOR = Symbol("OPS_AUTH_CODE_GENERATOR");
...
export class DomainPasswordlessAuthHandler {
  constructor(
    ...
    @Inject(OPS_AUTH_CODE_GENERATOR)
    private readonly codeGenerator: PasswordlessCodeGenerator,
    private readonly sessionTokenIssuer: OpsSessionTokenIssuer,
  ...
  async verifyCode(
    ...
    if (result.user.status !== "active") {
      throw new Error(`Authenticated astrologer account has unexpected status: ${result.user.status}`);
    }

    const roles = result.roleAssignments.map((assignment) => assignment.role);

    if (!roles.includes("astrologer")) {
      throw new Error("Authenticated account is missing the astrologer role");
    }
