...
@Injectable()
export class OpsCsrfTokenService {
  ...
  setCsrfCookie(input: {
    ...
    const tokenTtlMs =
      this.configService.getOrThrow<number>("opsApi.csrfTokenTtlSeconds") * 1000;
    ...
    input.response.cookie(this.configService.getOrThrow<string>("opsApi.csrfCookieName"), token, {
      httpOnly: false,
      secure: this.configService.getOrThrow<boolean>("opsApi.sessionCookieSecure"),
      sameSite: "lax",
      path: "/",
  ...
  clearCsrfCookie(response: CsrfCookieResponse): void {
    response.cookie(this.configService.getOrThrow<string>("opsApi.csrfCookieName"), "", {
      httpOnly: false,
      secure: this.configService.getOrThrow<boolean>("opsApi.sessionCookieSecure"),
      sameSite: "lax",
      path: "/",
  ...
  assertValidRequest(input: {
    ...
    const headerName = this.configService.getOrThrow<string>("opsApi.csrfHeaderName");
    ...
    const csrfCookie = readCookieValue(
      ...
      this.configService.getOrThrow<string>("opsApi.csrfCookieName")
    );
  ...
  private assertTrustedOrigin(request: CsrfRequest): void {
    const allowedOrigins =
      this.configService.getOrThrow<readonly string[]>("opsApi.allowedOrigins");
  ...
  private sign(input: {
    ...
    return createHmac("sha256", this.configService.getOrThrow<string>("opsApi.csrfSecret"))
      .update(
        [
          csrfTokenVersion,
