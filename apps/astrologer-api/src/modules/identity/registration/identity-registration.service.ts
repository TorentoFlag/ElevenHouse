...
import {
  ...
  type VerifyAstrologerRegistrationWithSessionResult
...
export class IdentityRegistrationService {
  ...
  async verifyCodeAndRegister(
    body: VerifyAstrologerRegistrationPasswordlessCodeRequest,
    context: PasswordlessRequestContext = {}
  ): Promise<VerifyAstrologerRegistrationWithSessionResult> {
    const request = verifyAstrologerRegistrationPasswordlessCodeRequestSchema.safeParse(body);

    if (!request.success) {
      throw new BadRequestException({
        message: "Invalid passwordless astrologer registration request",
    ...
    try {
      ...
      return {
        response: verifyAstrologerRegistrationPasswordlessCodeResponseSchema.parse(
          result.response
        ),
