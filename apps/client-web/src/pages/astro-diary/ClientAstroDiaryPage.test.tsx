...
describe("ClientAstroDiaryPage", () => {
  ...
  it("does not select or load a journal owned by a different route astrologer", async () => {
    ...
    renderPage();
    expect(await screen.findByText(clientCopyByLocale.en.astroDiary.noSubscriptionDescription)).toBeVisible();
  ...
  it("preserves entry text while stale authority is reloaded", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("textbox", { name: "Entry text" })).toHaveValue("Text that must survive"));
  });
  ...
  it("keeps write authority visibly pending until selected detail is confirmed", async () => {
    ...
    http.get.mockImplementation(async (path: string) => {
      ...
      if (path.startsWith(`/astro-diary/journals/${journalId}/timeline?`)) return { items: [], nextCursor: null, visibleMaxCursor: 0, hasMore: false };
      throw new Error(`Unexpected GET ${path}`);
    });
    renderPage();
  ...
  it("creates, updates, and publishes only a server-acknowledged draft", async () => {
    ...
    http.get.mockImplementation(async (path: string) => {
      ...
      if (path === `/astro-diary/journals/${journalId}`) return { ...activeSummary, journal: { ...activeSummary.journal, version: journalVersion } };
      if (path === `/astro-diary/journals/${journalId}/client-entry/draft`) return { draft: serverDraft };
      if (path.startsWith(`/astro-diary/journals/${journalId}/timeline?`)) return { items: [], nextCursor: null, visibleMaxCursor: 0, hasMore: false };
      throw new Error(`Unexpected GET ${path}`);
    });
    ...
    fireEvent.click(screen.getByRole("button", { name: "Save draft" }));
    await waitFor(() => expect(http.post).toHaveBeenCalledWith(expect.stringMatching(/\/client-entry\/drafts$/), expect.objectContaining({ body: "First version", expectedJournalVersion: 4 }), expect.anything()));

    fireEvent.change(textbox, { target: { value: "Second version" } });
    fireEvent.click(screen.getByRole("button", { name: "Save draft" }));
    await waitFor(() => expect(http.put).toHaveBeenCalledWith(expect.stringContaining(draftId), expect.objectContaining({ body: "Second version", expectedDraftVersion: 1 }), expect.anything()));

    await waitFor(() => expect(screen.getByRole("button", { name: "Publish entry" })).toBeEnabled());
    fireEvent.click(screen.getByRole("button", { name: "Publish entry" }));
    await waitFor(() => expect(http.post).toHaveBeenCalledWith(expect.stringMatching(/\/publish$/), expect.objectContaining({ expectedDraftVersion: 2 }), expect.anything()));
  });
});
...
function renderPage() {
  ...
  return render(
    <QueryClientProvider client={queryClient}>
      <I18nProvider dictionaries={clientCopyByLocale} initialLocale="en" storage={null} documentElement={null}>
        <MemoryRouter initialEntries={[`/me/astrologers/${astrologerId}/journal`]}>
          <Routes>
            <Route path={clientRouteContract.authenticatedAstroDiary} element={<ClientAstroDiaryPage />} />
          </Routes>
        </MemoryRouter>
      </I18nProvider>
    </QueryClientProvider>
...
const overview = {
  ...
  summary: { activeSubscriptionCount: 1, availableMaterialCount: 0, directLinkOnly: true, unreadNotificationCount: 0, upcomingBookingCount: 0 }
};
const activeSummary = {
  journal: { id: journalId, relationshipId: "31111111-1111-4111-8111-111111111111", journalEpochId: "51111111-1111-4111-8111-111111111111", astrologerUserId: astrologerId, clientUserId: "61111111-1111-4111-8111-111111111111", state: "active", version: 4, createdAt: "2026-08-18T10:00:00.000Z" },
  currentCycle: null,
  currentObligation: null,
  access: { mode: "active", subscriptionId: "71111111-1111-4111-8111-111111111111", subscriptionState: "active", currentPeriod: { id: "81111111-1111-4111-8111-111111111111", sequence: 1, startsAt: "2026-08-01T00:00:00.000Z", endsAt: "2026-09-01T00:00:00.000Z" }, allowance: { periodId: "81111111-1111-4111-8111-111111111111", total: 2, available: 2, reserved: 0, consumed: 0, released: 0 } },
  unreadCount: 0,
  visibleMaxCursor: 0
} satisfies AstroDiaryJournalSummaryResponse;
const otherSummary = { ...activeSummary, journal: { ...activeSummary.journal, id: "91111111-1111-4111-8111-111111111111", astrologerUserId: "a1111111-1111-4111-8111-111111111111" } } satisfies AstroDiaryJournalSummaryResponse;
