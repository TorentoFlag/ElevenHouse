...
export function usePasswordlessAuthFlowHandlers(input: {
  ...
  readonly challenge: RequestPasswordlessCodeResponse | null;
  ...
  readonly setChallenge: Dispatch<SetStateAction<RequestPasswordlessCodeResponse | null>>;
  ...
  readonly setPendingCredential: Dispatch<
    SetStateAction<PasswordlessPendingCredential | null>
  >;
  ...
  const handleCredentialSubmit = useCallback(async () => {
    ...
    try {
      ...
      requestAnimationFrame(() =>
        input.codeInputRef.current?.focus({ preventScroll: true })
      );
    } catch (error) {
