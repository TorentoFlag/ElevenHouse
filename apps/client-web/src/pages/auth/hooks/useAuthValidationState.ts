...
export function useAuthValidationState(input: {
  ...
  const nameError =
    showNameError && hasNameValidationError ? input.copy.validation.name : null;
  ...
  const hasPhoneValidationError =
    input.values.phone.length > 0 && !phoneValidation.valid;
  const phoneError =
  ...
  const credentialValues = {
    ...
    normalizedPhone:
      phoneValidation.normalizedValue ?? input.phoneInputState.normalizedValue,
