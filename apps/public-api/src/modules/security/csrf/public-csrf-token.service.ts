...
export class PublicCsrfTokenService {
  ...
  setCsrfCookie(input: {
    ...
    const tokenTtlMs =
      this.configService.getOrThrow<number>("publicApi.csrfTokenTtlSeconds") * 1000;
    ...
    input.response.cookie(this.configService.getOrThrow<string>("publicApi.csrfCookieName"), token, {
      httpOnly: false,
      secure: this.configService.getOrThrow<boolean>("publicApi.sessionCookieSecure"),
      sameSite: "lax",
      path: "/",
  ...
  clearCsrfCookie(response: CsrfCookieResponse): void {
    response.cookie(this.configService.getOrThrow<string>("publicApi.csrfCookieName"), "", {
      httpOnly: false,
      secure: this.configService.getOrThrow<boolean>("publicApi.sessionCookieSecure"),
      sameSite: "lax",
      path: "/",
  ...
  assertValidRequest(input: {
    ...
    const headerName = this.configService.getOrThrow<string>("publicApi.csrfHeaderName");
    ...
    const csrfCookie = readCookieValue(
      ...
      this.configService.getOrThrow<string>("publicApi.csrfCookieName")
    );
  ...
  private assertTrustedOrigin(request: CsrfRequest): void {
    const allowedOrigins =
      this.configService.getOrThrow<readonly string[]>("publicApi.allowedOrigins");
  ...
  private sign(input: {
    ...
    return createHmac(
      "sha256",
      this.configService.getOrThrow<string>("publicApi.csrfSecret")
    )
      .update(
        [
          csrfTokenVersion,
